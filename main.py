...
from analysis import extract_sin
...
if __name__ == '__main__':
    ...
    symphony = build_symphony(mod_X_m, phase_X_m, w_lp[m], envelope, f_e)
    ...
    plotting_args = [add_plotting_args('plot', None, X_m, 'FFT du signal x[n] de la guitare', 'n(échantillons)', 'amplitude'),
                     ...
                     add_plotting_args('stem', range(int(-N_low_pass / 2) + 1, int(N_low_pass / 2)), h_n_lp, f'réponse impulsionnelle du filtre passe-bas N={N_low_pass}', 'n(échantillons)', 'amplitude'),
                     ...
                     add_plotting_args('plot', None, envelope, 'envelope du signal x[n] de la guitare', 'n(échantillons)', 'amplitude'),
                     ...
                     add_plotting_args('plot', None, h_n_bs, f'réponse impulsionnelle du filtre passe-bas N={N_band_stop}', 'n(échatillons)', 'amplitude'),
                     add_plotting_args('plot', w_bs, 20 * np.log10(mod_H_m_bs), f'réponse fréquencielle du filtre passe-bas N={N_band_stop}', 'w(rad/échantillons)', 'amplitude(dB)'),
